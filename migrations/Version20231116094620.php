<?php

declare(strict_types=1);

namespace DoctrineMigrations;

use Doctrine\DBAL\Schema\Schema;
use Doctrine\Migrations\AbstractMigration;

/**
 * Auto-generated Migration: Please modify to your needs!
 */
final class Version20231116094620 extends AbstractMigration
{
    public function getDescription(): string
    {
        return '';
    }

    public function up(Schema $schema): void
    {
        // this up() migration is auto-generated, please modify it to your needs
        $this->addSql('ALTER TABLE league_team DROP FOREIGN KEY FK_2E3F061E296CD8AE');
        $this->addSql('ALTER TABLE league_team DROP FOREIGN KEY FK_2E3F061E58AFC4DE');
        $this->addSql('DROP TABLE league_team');
        $this->addSql('ALTER TABLE selection ADD teams_id INT DEFAULT NULL, ADD leagues_id INT DEFAULT NULL');
        $this->addSql('ALTER TABLE selection ADD CONSTRAINT FK_96A50CD7D6365F12 FOREIGN KEY (teams_id) REFERENCES team (id)');
        $this->addSql('ALTER TABLE selection ADD CONSTRAINT FK_96A50CD78642ED32 FOREIGN KEY (leagues_id) REFERENCES league (id)');
        $this->addSql('CREATE INDEX IDX_96A50CD7D6365F12 ON selection (teams_id)');
        $this->addSql('CREATE INDEX IDX_96A50CD78642ED32 ON selection (leagues_id)');
    }

    public function down(Schema $schema): void
    {
        // this down() migration is auto-generated, please modify it to your needs
        $this->addSql('CREATE TABLE league_team (league_id INT NOT NULL, team_id INT NOT NULL, INDEX IDX_2E3F061E58AFC4DE (league_id), INDEX IDX_2E3F061E296CD8AE (team_id), PRIMARY KEY(league_id, team_id)) DEFAULT CHARACTER SET utf8mb4 COLLATE `utf8mb4_unicode_ci` ENGINE = InnoDB COMMENT = \'\' ');
        $this->addSql('ALTER TABLE league_team ADD CONSTRAINT FK_2E3F061E296CD8AE FOREIGN KEY (team_id) REFERENCES team (id) ON DELETE CASCADE');
        $this->addSql('ALTER TABLE league_team ADD CONSTRAINT FK_2E3F061E58AFC4DE FOREIGN KEY (league_id) REFERENCES league (id) ON DELETE CASCADE');
        $this->addSql('ALTER TABLE selection DROP FOREIGN KEY FK_96A50CD7D6365F12');
        $this->addSql('ALTER TABLE selection DROP FOREIGN KEY FK_96A50CD78642ED32');
        $this->addSql('DROP INDEX IDX_96A50CD7D6365F12 ON selection');
        $this->addSql('DROP INDEX IDX_96A50CD78642ED32 ON selection');
        $this->addSql('ALTER TABLE selection DROP teams_id, DROP leagues_id');
    }
}
